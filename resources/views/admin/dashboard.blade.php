...
$(function() {
    // Get context with jQuery - using jQuery's .get() method.
    var bestSellerCanvas = $('#bestSeller').get(0).getContext('2d');
    // This will get the first returned node in the jQuery collection.
    var bestSeller = new Chart(bestSellerCanvas);

    var dataChartCandlestick = [];
    @foreach($produkTerlaris as $produk)
        var totalTerjual = parseInt({{ $produk->total_terjual }});
        var dataPoint = {
            t: Date.now(),
            o: totalTerjual
        };
        dataChartCandlestick.push(dataPoint);
    @endforeach

    var bestSellerData = {
        datasets: [{
            data: dataChartCandlestick
        }]
    };
    ...
    bestSeller.Candlestick(bestSellerData, bestSellerOptions);
});
</script>
@endpush
