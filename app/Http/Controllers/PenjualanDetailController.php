...
class PenjualanDetailController extends Controller
{
    ...
    public function data($id)
    {
        ...
        foreach ($detail as $item) {
            $stok = Produk::where('id_produk', $item->id_produk)->first()->stok ?? 0;

            $row = array();
            $row['kode_produk'] = '<span class="label label-success">'. $item->produk['kode_produk'] .'</span';
            $row['nama_produk'] = $item->produk['nama_produk'];
            $row['harga_jual']  = 'Rp. '. format_uang($item->harga_jual);
            $row['jumlah']      = '<input type="text" class="form-control input-sm quantity" id="qty-'. $item->id_penjualan_detail .'" data-id="'. $item->id_penjualan_detail .'" value="'. $item->jumlah.'" data-stok="'. $stok .'">';
